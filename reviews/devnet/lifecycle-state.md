...
| --- | --- | --- | --- | --- |
| `NODE-01` | 在全新隔离用户目录中，用受支持的本地 CKB 二进制启动 devnet daemon | 命令只在 direct RPC 与 proxy RPC 都通过真实节点健康检查后返回；JSON 结果包含 daemon PID、RPC、proxy、日志和 PID 文件位置；node、miner 与 chain spec 配置已生成，PID 状态为 running | 仅创建目录或打开端口就误报启动成功，或首次启动缺少必要配置 | P0 |
...
| `NODE-04` | 对健康运行的 daemon 执行 `node stop` | 返回 `stopped: true` 及原 daemon PID；node、miner、proxy 整个服务组退出，相关监听端口关闭且 PID 文件删除 | 只停止外层 CLI 而遗留 CKB、miner 或 proxy 进程 | P0 |
