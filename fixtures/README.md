...
这两个辅助程序从隔离安装的被测包解析 `@ckb-ccc/core` 公共 SDK；不导入 OffCKB 的 `src/`，也不替 OffCKB 生成 debugger 的完整交易文件。私钥只通过 `--privkey-file` 指定的文件读取，生成的请求 JSON 不携带私钥。pytest 负责准备输入、记录脱敏命令和核对链上结果，无需单独运行辅助程序。
